...
class AppStateRobotTeleopViewer(AppState):
    ...
    def draw_navmesh_lines(self, color: mn.Color4 = None) -> None:
        ...
        for p0, p1 in self.navmesh_lines:
            self._app_service.line_render.draw_transformed_line(p0, p1, color)
    ...
    def move_robot_on_navmesh(self) -> None:
        ...
        if self.robot is not None:
            ...
            if gui_input.get_key_down(GuiInput.KeyNS.M):
                self.robot.place_robot(
                    self._sim.pathfinder.get_random_navigable_point()
                )
    ...
    def handle_keys(
        ...
        if gui_input.get_key_down(GuiInput.KeyNS.O):
            if self.robot is not None:
    ...
    def handle_mouse_press(self) -> None:
        ...
        if (
            self._app_service.gui_input.get_mouse_button(GuiInput.MouseNS.LEFT)
    ...
    def sim_update(
        ...
        dblr = self._app_service.line_render
