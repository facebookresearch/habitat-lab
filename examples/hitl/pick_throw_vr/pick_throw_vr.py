...
class AppStatePickThrowVr(AppState):
    ...
    def __init__(self, app_service: AppService):
        ...
        self._target_obj_ids: Optional[List[int]] = None

        self._camera_helper: CameraHelper = CameraHelper(
        ...
        self._nav_helper: GuiNavigationHelper = GuiNavigationHelper(
            self._app_service, self.get_gui_controlled_agent_index()
        )
        self._throw_helper: GuiThrowHelper = GuiThrowHelper(
            self._app_service, self.get_gui_controlled_agent_index()
        )
        self._pick_helper: GuiPickHelper = GuiPickHelper(
            self._app_service,
            self.get_gui_controlled_agent_index(),
            self._get_gui_agent_feet_height(),
        )

        self._avatar_switch_helper: AvatarSwitcher = AvatarSwitcher(
