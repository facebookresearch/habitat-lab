...
PLACEHOLDER_EPISODE_COUNT = 2000
...
class AppStateStartSession(AppStateBase):
    ...
    def get_next_state(self) -> Optional[AppStateBase]:
        episode_indices = self._try_get_episode_indices(
            data=self._app_data,
            # TODO: Get session count from episode loader.
            total_episode_count=PLACEHOLDER_EPISODE_COUNT,
        )
        if episode_indices is not None:
            ...
            # If the session parameters are invalid, create a partial session record for data collection.
            self._new_session = Session(
                self._app_service.config,
                [],
                dict(self._app_data.connected_users),
