...
def play_env(env, args, config):
    ...
    waypoint_list = waypoint_generator(env, args, config)
    ...
    while True:
        ...
        if len(waypoint_list) > 0:
            reached_flag = reached(env.sim.robot.base_pos, waypoint_list[0][0], env.sim.robot.base_rot,  waypoint_list[0][1])
        if reached_flag and len(waypoint_list) > 0:
            waypoint_list = waypoint_list[1:]
            index += 1
        if (reached_flag or first_flag) and len(waypoint_list) > 0:
            agent = Controller()
