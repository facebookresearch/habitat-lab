...
if __name__ == "__main__":
    ...
    with habitat.Env(config=config) as env:
        env.reset()
        for i in range(50):
            observations = env.step({"action": 'agent_0_base_velocity', "action_args":{"agent_0_base_vel":[1.0,0.0]}})
            # im_0 = observations["head_rgb"]
            im_1 = observations["agent_0_third_rgb"]
            # output_im = np.concatenate((im_0, im_1), axis=1)
            images.append(im_1)
