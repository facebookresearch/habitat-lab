...
def play_env(env, args, config):
    ...
    continuous_path_follower = ContinuousPathFollower(env.sim, path, env.sim.robot.sim_obj.root_scene_node, waypoint_threshold=0.2)
    ...
    while True:
        ...
        agent = Controller()
        xyt_goal = [ continuous_path_follower.waypoint[0],  continuous_path_follower.waypoint[2],  float(env.sim.robot.base_rot)]
        ...
        if end_ep:
            ...
            if VIS_PATH:
                for vis_obj in vis_objs:
                    env.sim.get_rigid_object_manager().remove_object_by_id(vis_obj.object_id)
                vis_objs = setup_path_visualization(continuous_path_follower, env)
