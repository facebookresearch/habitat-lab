...
def get_input_vel_ctlr(
    ...
    args = {}
    if (
        base_action is not None
        and f"{agent_to_control}_BASE_VELOCITY" in env.action_space.spaces
    ):
        name = f"{agent_to_control}_BASE_VELOCITY"
        args = {f"{agent_to_control}_base_vel": base_action}
    else:
        name = f"{agent_to_control}_ARM_ACTION"
        if given_arm_action:
            # The grip is also contained in the provided action
            args = {
                f"{agent_to_control}_arm_action": arm_action[:-1],
                f"{agent_to_control}_grip_action": arm_action[-1],
            }
        else:
            args = {
                f"{agent_to_control}_arm_action": arm_action,
                f"{agent_to_control}_grip_action": magic_grasp,
            }
