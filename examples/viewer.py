...
class HabitatSimInteractiveViewer(Application):
    ...
    def __init__(self, sim_settings: Dict[str, Any]) -> None:
        ...
        # kinematic relationship manager
        self.krm = None
    ...
    def debug_draw(self):
        ...
        if self.mouse_grabber is not None:
            grabbed_obj = sutils.get_obj_from_id(
                self.sim, self.mouse_grabber.settings.object_id_a
            )
            debug_line_render.draw_transformed_line(
                grabbed_obj.translation,
                grabbed_obj.translation + mn.Vector3(0, -2, 0),
                mn.Color4.green(),
            )
        ...
        if self.mouse_pressed:
            ...
            if raycast_results.has_hits():
                ...
                debug_line_render.draw_circle(
                    translation=hit_info.point,
                    radius=0.01,
                    color=mn.Color4.yellow(),
                    normal=camera_position - hit_info.point,
                )
    ...
    def draw_event(
        ...
        if self.time_since_last_simulation >= 1.0 / self.fps:
            ...
            if self.mouse_grabber and not self.simulating:
                # we're in kinematic mode with an object, update the relationships
                self.krm.apply_relations()
                self.krm.update_snapshots(
                    root_parent_subset=[
                        self.mouse_grabber.settings.object_id_a
                    ]
                )
    ...
    def reconfigure_sim(self) -> None:
        ...
        self.krm = KinematicRelationshipManager(self.sim)
        self.krm.initialize_relationship_graph()
    ...
    def key_press_event(self, event: Application.KeyEvent) -> None:
        ...
        if key == pressed.ESC:
            ...
            self.semantic_region_debug_draw = (
                not self.semantic_region_debug_draw
            )
            ...
            curr_agent_transform = self.default_agent.scene_node.transformation
            curr_cam_transform = self.render_camera.node.transformation
            self.reconfigure_sim()
            logger.info(
                f"Reconfigured simulator for scene: {self.sim_settings['scene']}"
            )
            self.default_agent.scene_node.transformation = curr_agent_transform
            self.render_camera.node.transformation = curr_cam_transform
            ...
            if shift_pressed and self.cached_urdf:
                ...
                urdf_file_path = input(
                    "Load URDF: provide a URDF filepath:"
                ).strip()
    ...
    def mouse_press_event(self, event: Application.MouseEvent) -> None:
        ...
        if physics_enabled:
            ...
            if raycast_results.has_hits():
                ...
                print(f"mousecast hit object id {hit_info.object_id}")

                if (
                    hit_info.object_id > habitat_sim.stage_id
                    and self.mouse_interaction == MouseMode.GRAB
                ):
                    ...
                    if hit_object >= 0:
                        ...
                        if self.simulating:
                            ...
                            if constraint_settings.link_id_a == -1:
                                # remove the grabbed object from the relations manager so it can be manipulated
                                self.krm.relationship_graph.remove_obj_relations(
                                    constraint_settings.object_id_a,
                                    parents_only=True,
                                )
                                self.mouse_grabber = KinematicMouseGrabber(
                                    constraint_settings,
                                    grip_depth,
                                    self.sim,
                                )
                            else:
                                # this is link, call the open API
                                if sutils.link_is_open(ao, ao_link, 0.02):
                                    sutils.close_link(ao, ao_link)
                                else:
                                    sutils.open_link(ao, ao_link)
                                self.krm.apply_relations()
                                self.krm.update_snapshots(
                                    root_parent_subset=[hit_object]
                                )
                    else:
                        logger.warn(
...
Simulation Mode: {"DYNAMIC" if self.simulating else "KINEMATIC"}
            """
        )
...
class KinematicMouseGrabber(MouseGrabber):
    """
    Manipulate objects kinematically with the same interface as MouseGrabber.
    """
    ...
    def __init__(
        ...
        assert (
            settings.link_id_a == -1
        ), "Kinematic MouseGrabber cannot manipulate links. Use open/close APi instead."

        print(f"KinematicMouseGrabber grabbed {settings.object_id_a}")

        # defines distance of the grip point from the camera for pivot updates
        self.grip_depth = grip_depth
        self.global_mouse_grabber_transform = mn.Matrix4.from_(
            settings.frame_b, settings.pivot_b
        )
        self.relative_transformation = mn.Matrix4.from_(
            settings.frame_a, settings.pivot_a
        ).inverted()

    def __del__(self):
        """
        No-op for kinematic mode.
        """
    ...
    def _update_object_transform(self) -> None:
        """
        Apply the transform to the grabbed object.
        """
        obj = None
        rom = self.simulator.get_rigid_object_manager()
        aom = self.simulator.get_articulated_object_manager()
        if rom.get_library_has_id(self.settings.object_id_a):
            obj = rom.get_object_by_id(self.settings.object_id_a)
        else:
            # must be an ao
            obj = aom.get_object_by_id(self.settings.object_id_a)
        obj.transformation = (
            self.global_mouse_grabber_transform @ self.relative_transformation
        )

    def update_transform(self, transform: mn.Matrix4) -> None:
        self.global_mouse_grabber_transform = transform
    ...
    def rotate_local_frame_by_global_angle_axis(
        ...
        """rotate the object's local constraint frame with a global angle axis input."""
        local_axis = self.relative_transformation.transform_vector(axis)
        R = mn.Matrix4.rotation(angle, local_axis.normalized())
        new_rotation = R.rotation().__matmul__(
            self.relative_transformation.rotation()
        )
        self.relative_transformation = mn.Matrix4.from_(
            new_rotation, self.relative_transformation.translation
        )
        self._update_object_transform()
