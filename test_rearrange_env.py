...
def main():
    ...
    urdf_path = os.path.join(
        data_path, "robots/hab_murp/murp_tmr_franka/murp_tmr_franka_metahand.urdf"  #"robots/hab_spot_arm/urdf/hab_spot_arm.urdf"
    )
    ...
    start_position = np.array([2.0, 0.7, -1.0])
    start_rotation = -90
    ...
    for _ in range(100):
        action = {
            ...
            "action_args": {
                "base_vel": np.array([1.0, 0.0], dtype=np.float32)
            },
        }
        obs = env.step(action)
