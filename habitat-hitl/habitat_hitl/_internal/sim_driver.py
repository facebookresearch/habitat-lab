...
class SimDriver(AppDriver):
    def __init__(
        ...
        """
        self._enable_renderer = config["habitat_hitl"][
            "enable_sim_driver_renderer"
        ]
        ...
        cfg_settings["scene"] = "NONE"
        cfg_settings["depth_sensor"] = self._enable_renderer
        cfg_settings["color_sensor"] = self._enable_renderer
        ...
        assert (self.get_sim().renderer is not None) == self._enable_renderer
