...
class NetworkManager:
    ...
    def __init__(self, interprocess_record: InterprocessRecord):
        ...
        consolidated_messages: List[Message] = [{} for _ in range(self._max_client_count)]
    ...
    def _update_consolidated_keyframes_and_messages(self,
    ...
    def _check_kick_client(self, message: Message) -> bool:
            ...
            if len(inc_keyframes_and_messages) > 0:
                ...
                if len(inc_keyframes_and_messages) > 1:
                    for _ in range(1, len(inc_keyframes)):
                        self._update_consolidated_keyframes_and_messages(tmp_con_keyframe, inc_keyframes_and_messages[1:])
                ...
                self._update_consolidated_keyframes_and_messages(self._consolidated_keyframe_and_messages, inc_keyframes_and_messages)

                tasks = {}
                for user_index in self._user_slots.keys():
                    if self.is_okay_to_send_keyframes(user_index):
                        client = self._user_slots[user_index]
                        tasks[user_index] = client.socket.send(user_json_strings[user_index])
                        slot.recent_connection_activity_timestamp = (
                            datetime.now()
                        )
                # TODO: Asyncio gather or wait
                for user_index in self._user_slots.keys():
                    if self.is_okay_to_send_keyframes(user_index):
                        try:
