...
from habitat.tasks.nav.object_nav_thinking_task import ThoughtSensor
...
@baseline_registry.register_policy(name="PointNavResNetThinkingPolicy")
class PointNavResNetThinkingPolicy(NetPolicy):
    def __init__(
        self,
...
class AlignedCLIPEncoder(nn.Module):
    def __init__(
        self,
        observation_space: spaces.Dict,
    ):
        ...
        if not self.is_blind:
            ...
            self.output_shape = (1024,) if self.rgb and self.depth else (512,)
    ...
    def forward(self, observations: Dict[str, torch.Tensor]) -> Optional[torch.Tensor]:  # type: ignore
        if self.is_blind:
            return None
        ...
        if self.rgb:
            ...
            rgb_x = self.model.encode_image(rgb_observations).float()
            cnn_input.append(rgb_x)
        ...
        if self.depth:
            ...
            depth_x = self.model.encode_image(ddd).float()
            cnn_input.append(depth_x)
...
class PointNavResNetNet(Net):
    ...
    def __init__(
        ...
        if fuse_keys is None:
            ...
            # removing keys for auxiliary sensors like ThoughtSensor
            aux_sensor_keys = {
                ThoughtSensor.cls_uuid,
            }
            fuse_keys = [k for k in fuse_keys if k not in goal_sensor_keys and k not in aux_sensor_keys]
        ...
        if ThoughtSensor.cls_uuid in observation_space.spaces:
            rnn_input_size += 512
        ...
        if backbone == 'aligned_clip':
            self.visual_encoder = AlignedCLIPEncoder(
                observation_space
    ...
    def forward(
        ...
        if ThoughtSensor.cls_uuid in observations:
            thought_feats = observations[ThoughtSensor.cls_uuid]
            x.append(thought_feats.float())
