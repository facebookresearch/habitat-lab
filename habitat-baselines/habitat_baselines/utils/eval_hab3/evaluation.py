...
def get_results_from_folder(folder_name):
    ...
    for file_name in all_files:
        ...
        results_dict[curr_result["id"]] = EpisodeResults(
            curr_result["summary"]
        )
    return results_dict
        ...
        for episode_id in episode_ids:
            ...
            for eval_result in list_eval_results:
                if episode_id not in eval_result.results_run:
                    if self.debug:
                        print(
                            f"Missing episode {episode_id} in file {eval_result.folder_name}"
                        )
                    continue
...
def compute_all_metrics():
    ...
    print_pretty = True
    # Single agent runs:
    base_path = "/data/home/akshararai/hab3/habitat-lab/eval_test_data/"
    single_agent = "solo_eval_data/"
    ###############################
    # Get the single agent_run
    results_single_agent_seed = []
    for seed in range(3):
        for run_single_agent_file in glob.glob(f"{base_path}/{single_agent}"):
    ...
    for seed_number in range(3):
        ...
        metrics_aggregated["train"]["SpeedUp"].append(
            aggregated_results_single_agent.compute_speedup(results_train_pop)[
                0
            ]
        )
        ...
        metrics_aggregated["val"]["SpeedUp"].append(
            aggregated_results_single_agent.compute_speedup(results_val_pop)[0]
        )
    ...
    print_metrics_aggregated(
        metrics_aggregated, print_latex=print_latex, print_pretty=print_pretty
    )
    breakpoint()
