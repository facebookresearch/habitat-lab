...
class EvalRunResults:
    def __init__(
        ...
        eval_results=[],
        debug=False,
        add_split_name=False,
    ):
        self.debug = debug
        if folder_names is not None:
            self.results_run = get_results_from_folders(
                folder_names, add_split_name
            )
    ...
    def average_success_rate(self):
        return self.average_metrics(metric_names=["composite_success"])[
            "composite_success"
        ]
    ...
    def average_metrics(self, metric_names=["composite_success", "num_steps"]):
        ...
        for episode_id, episode_result in self.results_run.items():
            for metric_name in metric_names:
                metric_list[metric_name].append(
                    episode_result.episode_dict[metric_name]
                )
    ...
    def compute_speedup(self, cooperative_result, count_fail=True):
        ...
        for episode_id in base_eval_results.results_run:
            ...
            if count_fail:
                curr_speedup = (
                    success_multi * time_single_agent * 1.0 / time_multi_agent
                )
        ...
        for episode_id in episode_ids:
            ...
            for eval_result in list_eval_results:
                if episode_id not in eval_result.results_run:
                    if self.debug:
                        print(
                            f"Missing episode {episode_id} in file {eval_result.folder_names}"
                        )
                    continue
                results_from_episode.append(
                    eval_result.results_run[episode_id]
                )
            for key_metric in metrics:
                metric_list = [
                    result_episode.episode_dict[key_metric]
                    for result_episode in results_from_episode
                ]
...
def print_metrics_aggregated(
    metrics_aggregated, print_latex=False, print_pretty=False
):
    latex_res = []
            ...
            if print_pretty:
                print(
                    "{}.{} - mean: {:.02f} std: {:.02f}".format(
                        split, metric_name, np.mean(values), np.std(values)
                    )
                )
                print(
                    "{}.{} - mean: {:.02f} std: {:.02f}".format(
...
def compute_all_metrics():
    ...
    if not HITL:
        ...
        for seed in range(3):
            for run_single_agent_file in glob.glob(
                f"{base_path}/{single_agent}/{seed}/eval_data"
            ):
                results_single_agent_seed.append(
                    EvalRunResults([run_single_agent_file])
                )

        aggregated_results_single_agent = EvalRunResults(
            eval_results=results_single_agent_seed
        )
        ##############################
        ...
        for seed_number in range(3):
            ...
            results_train_pop = EvalRunResults(
                [f"{path_run}/episode_data_train_pop"]
            )
            results_val_pop = EvalRunResults(
                [f"{path_run}/episode_data_val2_pop"]
            )
            metrics_aggregated = {
                "train": {"SR": [], "SpeedUp": []},
                "val": {"SR": [], "SpeedUp": []},
            }
            metrics_aggregated["train"]["SpeedUp"].append(
                aggregated_results_single_agent.compute_speedup(
                    results_train_pop
                )[0]
            )
            metrics_aggregated["train"]["SR"].append(
                results_train_pop.average_success_rate[0]
            )

            metrics_aggregated["val"]["SpeedUp"].append(
                aggregated_results_single_agent.compute_speedup(
                    results_val_pop
                )[0]
            )
            metrics_aggregated["val"]["SR"].append(
                results_val_pop.average_success_rate[0]
            )

        print_metrics_aggregated(
            metrics_aggregated,
        ...
        metrics_aggregated = {"val": {"SR": [], "SpeedUp": []}}
        # Planner-Planner
        ...
        dataset_splits = [
            "microtrain_eval_small_1scene_2objs_30epi_45degree_pop",
        ...
        for seed in range(3):
            seed_folders = [
                (
                    f"{fn}/{seed}/eval_data",
                    split.replace("_1scene_2objs_30epi_45degree_pop", ""),
                )
                for fn, split in zip(folders, dataset_splits)
            ]

            results_single_agent_seed.append(
                EvalRunResults(seed_folders, add_split_name=True)
            )
        ...
        for seed_number in range(3):
            ...
            results_val_pop = EvalRunResults(
                [
                    (
                        f"{path_run}/episode_data_val_{split}",
                        split.replace("_1scene_2objs_30epi_45degree_pop", ""),
                    )
        ...
        for plan_idx in range(4):
            ...
            for seed_number in range(3):
                ...
                results_val_pop = EvalRunResults(
                    [
                        (
                            f"{path_run}/episode_data_val_{split}",
                            split.replace(
                                "_1scene_2objs_30epi_45degree_pop", ""
                            ),
                        )
                        for split in dataset_splits
                    ],
                ...
                metrics_aggregated["val"]["SpeedUpSuccess"].append(
                    aggregated_results_single_agent.compute_speedup(
                        results_val_pop, count_fail=False
                    )[0]
                )
        ...
        for seed_number in range(3):
            ...
            results_val_pop = EvalRunResults(
                [
                    (
                        f"{path_run}/episode_data_{split}",
                        split.replace("_1scene_2objs_30epi_45degree_pop", ""),
                    )
                    for split in dataset_splits
                ],
