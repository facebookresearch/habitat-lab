...
class ExpertDatagen:
    def __init__(self, ep_id=0, mode="train", retract=False):
        if mode == "train":
            self.json_path = (
        ...
        video_dir = f"{self.save_path}_videos"
        print("video_dir: ", video_dir)
        os.makedirs(video_dir, exist_ok=True)
        self.video_path = f"{video_dir}/ep_{ep_id}.mp4"
    ...
    def add_episode_data(self, obs, action):
        ...
        ep_data = {
            "joints": np.array(curr_joints[:7]).tolist(),
    ...
    def move_to_joint(
        ...
        while not np.allclose(
            ...
            save_action = {
                ...
                "base_action": [0, 0],
                "empty_action": [0],
                "grip_action": [1] if curr_joint_pos[-1] > -1.4 else [-1],
            }
            if save_info:
                self.add_episode_data(obs, save_action)
            if ctr > timeout:
    ...
    def nav_to_obj(self):
        ...
        while dist > 0.5 and i < 200:
            ...
            lin_vel_norm = (
                action_planner["action_args"]["base_vel"][0]
                / nav_planner.forward_velocity
            )
            ang_vel_norm = (
                action_planner["action_args"]["base_vel"][1]
                / nav_planner.turn_velocity
            )

            save_action = {
                "arm_action": np.array(curr_ee_pos).tolist(),
                "base_action": np.array([lin_vel_norm, ang_vel_norm]).tolist(),
                "empty_action": [0],
                "grip_action": [-1],
            }
    ...
    def retract_arm(self, save_info=True, save_imgs=True):
    ...
    def run_expert(self):
        ...
        if self.retract:
            self.retract_arm(save_info=self.retract, save_imgs=self.retract)
...
if __name__ == "__main__":
    ...
    datagen = ExpertDatagen(args.ep_id, args.mode, args.retract)
    datagen.run_expert()
