...
def find_file(folder, filename):
    ...
    print("result: ", result, folder, filename)
...
def convert_object_glb_to_usd(
    ...
    # files = find_all_files(objects_root_folder, ".glb")
    collision_asset_filepath = objects_root_folder
    render_asset_filepath = objects_root_folder
    # collision_asset_filepath = [f for f in files if "collider" in f][0]
    # render_asset_filepath = [f for f in files if "collider" not in f][0]

    print(f"Converting {collision_asset_filepath}...")
    print(f"out_usd_path {out_usd_path}...")
    ...
    render_asset_filepath_from_urdf = render_asset_filepath

    """
    (Pdb) render_asset_filepath_from_urdf = object_config_json_data["render_asset"]
    (Pdb) os.path.abspath( os.path.join(object_config_folder, render_asset_filepath_from_urdf) ),
    ('/opt/hpcaas/.mounts/fs-03ee9f8c6dddfba21/jtruong/repos/vla-physics/habitat-lab/data/objects/ycb/meshes/063-b_marbles/google_16k/textured.glb',)
    (Pdb) os.path.relpath( os.path.abspath( os.path.join(object_config_folder, render_asset_filepath_from_urdf) ), start=project_root_folder, )
    'data/objects/ycb/meshes/063-b_marbles/google_16k/textured.glb'
    (Pdb) object_config_folder
    '/opt/hpcaas/.mounts/fs-03ee9f8c6dddfba21/jtruong/repos/vla-physics/habitat-lab/data/objects/ycb/configs'
    (Pdb) render_asset_filepath_from_urdf
    '../meshes/063-b_marbles/google_16k/textured.glb'
    """
    render_asset_filepath_for_usd = os.path.relpath(
        os.path.abspath(
            os.path.join(
                render_asset_filepath, render_asset_filepath_from_urdf
            )
        ),
        start=project_root_folder,
    )

    render_asset_scale = (1.0, 1.0, 1.0)
...
def batch_convert_object_glb_to_usd(
    input_dir, output_dir, project_root_folder
):
    for root, dirs, files in os.walk(input_dir):
        for file in files:
            if file.endswith("textured.glb"):
...
def convert_hab_scene(
    scene_filepath, project_root_folder, enable_collision_for_stage=True
):
    ...
    assert os.path.exists(objects_root_folder) and os.path.isdir(
        objects_root_folder
    )
    ...
    for obj_instance_json in scene_json_data["articulated_object_instances"]:
        ...
        if urdf_filepath:
            ...
            base_object_name = os.path.splitext(
                os.path.basename(collision_asset_rel_filepath)
            )[0]
            ...
            if not os.path.exists(out_usd_path):
                try:
                    convert_object_urdf(
                        collision_asset_rel_filepath, out_usd_path
                    )
                except Exception as e:
                    print(
                        f"Error converting URDF to USD for {urdf_filename}: {e}"
                    )
                    continue
        ...
        if not os.path.exists(out_usd_path):
            try:
                object_config_filepath = find_file(
                    objects_root_folder, object_config_filename
                )
                ...
                print(
                    "skipping: ", objects_root_folder, object_config_filename
                )
...
if __name__ == "__main__":
    ...
    # convert_hab_scene(
    #     "data/Fremont-Knuckles/configs/scenes/fremont_static_v2.scene_instance.json",
    #     project_root_folder="./",
    #     enable_collision_for_stage=True,
    # )
    #     convert_hab_scene(
    ...
    #        enable_collision_for_stage=True,
    #    )
    batch_convert_object_glb_to_usd(
        "data/sim_assets_all/assets/dexgraspnet2/meshdata_glb",
        "data/sim_assets_all/assets/dexgraspnet2/meshdata_usda",
        "./",
    )
