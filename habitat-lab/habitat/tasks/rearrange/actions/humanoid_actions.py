...
class HumanoidPickAction(HumanoidJointAction):
    def __init__(self, *args, task, **kwargs):
        ...
        self.hand_state = 0  # 0 for reset, 1 for approaching
    ...
    def step(self, *args, is_last_action, **kwargs):
        ...
        if object_pick_idx not in self._object_coord:
            self._object_coord[object_pick_idx] = self._get_coord_for_idx(
                object_pick_idx
            )
        object_coord = self._object_coord[object_pick_idx]
        ...
        dist_to_object = np.linalg.norm(object_coord - init_coord_world)
        if dist_to_object > 1.5:
            return {}
        hand_vector = (object_coord - init_coord_world) / np.linalg.norm(
            object_coord - init_coord_world
        )
        max_num_iters = int(
            np.linalg.norm(object_coord - init_coord_world) / dist_move
        )
        ...
        if self.hand_state == 0:  # Approaching
            ...
            if should_pick == 0 or self.cur_grasp_mgr.snap_idx is None:
                new_hand_coord = (
                    init_coord_world
                    + self.hand_pose_iter * dist_move * hand_vector
                )
                ...
                print(self.hand_pose_iter, max_num_iters, object_coord)
                if (
                    dist_hand_obj < 0.02
                    or self.hand_pose_iter == max_num_iters
                ):
                    # snap
                    self.hand_state = 1
                    if should_pick:
