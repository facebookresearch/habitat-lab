...
class IsaacRearrangeSim(HabitatSim):
    ...
    def _load_navmesh(self, ep_info: RearrangeEpisode):
        navmesh_path = ep_info.navmesh_path
        if len(navmesh_path) == 0:
            # TODO: copied here because import errors, make it a util
            def find_file(folder: str, filename: str, find_first: bool) -> str:
                """
                Find a particular file by name via recursive search in a root directory.
                Returns the absolute path of the file.
                NOTE: find_first returns the first match instead of asserting uniqueness
                """
                result = None
                for root, _dirs, files in os.walk(folder):
                    if filename in files:
                        assert (
                            not result
                        ), f"multiple matches found. First is '{result}', second is '{os.path.join(root, filename)}'"
                        result = os.path.join(root, filename)
                        if find_first:
                            break
                return os.path.abspath(result)

            # no navmesh path configured, try to find one in the dataset
            scene_name = ep_info.scene_id.split("/")[-1].split(".")[0]
            print(f"scene_name = {scene_name}")
            data_dir = os.path.dirname(ep_info.scene_dataset_config)
            navmesh_filename = scene_name + ".navmesh"
            print(f"navmesh_filename = {navmesh_filename}")
            print(f"data_dir = {data_dir}")
            navmesh_path = find_file(
                data_dir, navmesh_filename, find_first=True
            )
            print(navmesh_path)
