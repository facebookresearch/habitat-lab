...
class RearrangeEpisodeGenerator:
    ...
    def _get_resource_sets(self) -> None:
        ...
        for scene_set in self.cfg.scene_sets:
            ...
            self._scene_sets[scene_set["name"]] = (
        ...
        for object_set in self.cfg.object_sets:
            ...
            self._obj_sets[object_set["name"]] = (
                cull_string_list_by_substrings(
    ...
    def _get_obj_samplers(self) -> None:
        ...
        for obj_sampler_info in self.cfg.object_samplers:
            ...
            if obj_sampler_info["type"] == "uniform":
                ...
                self._obj_samplers[obj_sampler_info["name"]] = (
            ...
            if target_sampler_info["type"] == "uniform":
                ...
                self._target_samplers[target_sampler_info["name"]] = (
                    samplers.ObjectTargetSampler(
                        ...
                        recep_set_sample_probs=target_sampler_info[
                            "params"
                        ].get("sample_probs", None),
    ...
    def generate_episodes(
        ...
        while len(generated_episodes) + failed_episodes < num_episodes:
            print(
                "num_episodes: ",
                num_episodes,
                "generated_episodes",
            ...
            try:
                self._scene_sampler.set_cur_episode(
                    len(generated_episodes) + failed_episodes
                )
                # new_episode = self.generate_single_episode()
                new_episode = self.generate_single_arm_episode()
            except Exception as e:
                new_episode = None
                logger.error("Generation failed with exception...: ", e)
    ...
    def generate_single_episode(self) -> Optional[RearrangeEpisode]:
        ...
        for sampler_name, target_sampler in self._target_samplers.items():
            ...
            for i, (instance_handle, value) in enumerate(
                ...
                self.episode_data["sampled_targets"][instance_handle] = (
                    np.array(target_transform)
                )
                target_refs[instance_handle] = (
    ...
    def generate_single_arm_episode(
        self, use_floor=True, floor_prob=1.0
        ...
        generate_floor_episode = use_floor and random.random() < floor_prob
        ...
        # sample and allocate receptacles to contain the target objects
        target_receptacles = defaultdict(list)
        floor_receptacles = defaultdict(list)
        all_target_receptacles = []
        all_floor_receptacles = []

        floor_receptacle = None
        while floor_receptacle is None:
            try:
                receptacles = find_receptacles(self.sim)
                floor_receptacle = receptacles[0]
            except Exception as e:
                pass
        floor_receptacle.name = "floor"
        ...
        for sampler_name, num_targets in target_numbers.items():
            ...
            max_tries = 100
            ctr = 0
            new_floor_receptacles = []
            num_receptacles = (
                len(new_target_receptacles) + len(new_floor_receptacles)
                if generate_floor_episode
                else len(new_target_receptacles)
            )
            while num_receptacles < num_targets:
                num_receptacles = (
                    len(new_target_receptacles) + len(new_floor_receptacles)
                    if generate_floor_episode
                    else len(new_target_receptacles)
                ...
                if ctr > max_tries and len(new_receptacle) == 0:
                    print("Exceeded max tries with ctr: ", ctr)
                    return None
                if generate_floor_episode:
                    # if use_floor and False:
                    obj_sampler_name = "any"
                    new_floor_receptacles = "floor"
                    floor_receptacles["any"].extend(new_target_receptacles)
                    all_floor_receptacles.extend(new_target_receptacles)
                    ctr += 1
                else:
                    obj_sampler_name = random.choice(
                        targ_sampler_name_to_obj_sampler_names[sampler_name]
                    ...
                    try:
                        ...
                        print(f"New Receptacle Sample Failed: {e}")
                        # No receptacle instances found matching this sampler's requirements, likely ran out of allocations and a different sampler should be tried
                        failed_samplers[obj_sampler_name]
                        continue
                    ...
                    target_receptacles[obj_sampler_name].extend(
        ...
        for sampler_name, obj_sampler in self._obj_samplers.items():
            if generate_floor_episode:
                ...
                num_pairing_tries = 0
                while (
                    len(object_sample_data) < obj_sampler.target_objects_number
                    and num_pairing_tries < obj_sampler.max_sample_attempts
                ):
                    num_pairing_tries += 1
                    new_object_handle = obj_sampler.sample_object()
                    new_object = self.sample_object_placement_on_floor(
                        object_handle=new_object_handle,
                    )
                    object_sample_data.append((new_object, floor_receptacle))
            else:
                object_sample_data = obj_sampler.sample(
                    self.sim,
                    recep_tracker,
        ...
        for sampler_name, target_sampler in self._target_samplers.items():
            ...
            if generate_floor_episode:
                new_target_objects = {}
                for use_target in target_sampler.object_instance_set:
                    new_object = self.sample_object_placement_on_floor(
                        object_handle=use_target.creation_attributes.handle,
                    )
                ...
                new_target_objects = target_sampler.sample(
                    ...
                    vdb=self.vdb,
                    target_receptacles=target_receptacles[obj_sampler_name],
                    goal_receptacles=target_receptacles[obj_sampler_name],
                    # goal_receptacles=goal_receptacles[sampler_name],
                    object_to_containing_receptacle=self.object_to_containing_receptacle,
                )
            ...
            for i, (instance_handle, value) in enumerate(
                ...
                target_object, target_receptacle = value
                if target_receptacle.name != "floor":
                    target_receptacles[obj_sampler_name][i] = target_receptacle
                assert (
                    instance_handle not in self.episode_data["sampled_targets"]
                ), f"Duplicate target for instance '{instance_handle}'."
        ...
        return RearrangeEpisode(
            ...
            targets=self.episode_data["sampled_targets"],
            target_receptacles=save_target_receps,
            goal_receptacles=save_target_receps,
            markers=self.cfg.markers,
            name_to_receptacle=name_to_receptacle,
            info={"object_labels": target_refs},
        )
