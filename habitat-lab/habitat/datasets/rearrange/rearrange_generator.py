...
class RearrangeEpisodeGenerator:
    ...
    def visualize_scene_receptacles(self) -> None:
        ...
        for receptacle in receptacles:
            ...
            self.vdb.look_at(
                receptacle.sample_uniform_global(self.sim, 1.0)
            )
    ...
    def initialize_sim(self, scene_name: str, dataset_path: str) -> None:
        ...
        output_path = "rearrange_ep_gen_output/" if self.vdb is None else self.vdb.output_path
        self.vdb = DebugVisualizer(
            self.sim, output_path=output_path
        )
    ...
    def settle_sim(
        ...
        for new_object in self.ep_sampled_objects:
            ...
            if error > error_eps:
                ...
                if self._render_debug_obs:
                    self.vdb.peek_rigid_object(obj=new_object, peek_all_axis=True, additional_savefile_prefix="unstable_", debug_lines=[([spawn_positions[new_object.handle], new_object.translation], mn.Color4.red())])
        logger.info(
        ...
        logger.info(
            f"  Detailed sampling stats:"
        )
        rec_num_obj_vs_unstable: Dict[List[int]] = {}#receptacle_name: [num_objects, num_unstable_objects]
        ...
        if self.cfg.correct_unstable_results and not success:
            logger.info(
                f"  attempting to correct unstable placements..."
            )
            for sampler_name, objects in self.episode_data["sampled_objects"].items():
                ...
                unstable_subset = [obj_name for obj_name in unstable_placements if obj_name in obj_names]
                #check that we have freedom to reject some objects
                if len(objects)-len(unstable_subset) >= sampler.num_objects[0]:
                    #remove the unstable objects from datastructures
                    self.episode_data["sampled_objects"][sampler_name] = [obj for obj in self.episode_data["sampled_objects"][sampler_name] if obj.handle not in unstable_subset]
                    self.ep_sampled_objects = [obj for obj in self.ep_sampled_objects if obj.handle not in unstable_subset]
                else:
                    logger.info(
