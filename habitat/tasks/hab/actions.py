...
from habitat.tasks.hab.envs.utils import rearrang_collision
...
class ArmAction(SimulatorTaskAction):
    ...
    def __init__(self, *args, config, sim, **kwargs):
        ...
        self.grip_ctrlr = grip_controller_cls(*args, config=config, sim=sim, **kwargs)
...
class MagicGraspAction(SimulatorTaskAction):
    ...
    def _grasp(self):
        ...
        if self.snap_markers and len(marker_name_pos) > 0:
            ...
            closest_marker_idx = np.argmin(np.linalg.norm(ee_pos - marker_pos,
                axis=-1))
...
class BaseVelAction(SimulatorTaskAction):
    ...
    def update_base(self):
        ...
        end_pos = sim.step_filter(rigid_state.translation, target_rigid_state.translation)
    ...
    def step(self, base_vel, should_step=True, **kwargs):
        ...
        if not self.allow_back:
            raise ValueError("Not allowing backwards does not work with detecting backwards action")
