...
def action_list_to_one_hot(action_list):
...
class GreedyActionPathFollower:
    ...
    def get_next_action(self, goal_pos: np.array) -> np.array:
        if np.linalg.norm(goal_pos - self._sim.get_agent_state().position) <= self._goal_radius:
            return action_list_to_one_hot([SIM_NAME_TO_ACTION[SimulatorActions.STOP.value]])

        max_grad_dir = self._est_max_grad_dir(goal_pos)
        if max_grad_dir is None:
            return action_list_to_one_hot([SIM_NAME_TO_ACTION[SimulatorActions.FORWARD.value]])
        return self.step_along_grad(max_grad_dir, goal_pos)

    def step_along_grad(self, grad_dir: np.quaternion, goal_pos: np.array) -> np.array:
        current_state = self._sim.get_agent_state()
        alpha = self._angle_between_quats(grad_dir, v_to_q(current_state.rotation))
        if alpha <= np.deg2rad(self._sim.config.TURN_ANGLE) + 1e-3:
            ...
            best_turn = [(
                ...
                if (self._angle_between_quats(grad_dir, v_to_q(self._sim.get_agent_state().rotation)) < alpha)
                else SIM_NAME_TO_ACTION[SimulatorActions.RIGHT.value]
            )]
            ...
            movement_size = np.sqrt(np.sum(np.square(new_pos - current_state.position)))
            self._reset_agent_state(current_state)
            if new_dist < curr_dist and movement_size / self._step_size > 0.99:
                return action_list_to_one_hot(SIM_NAME_TO_ACTION[SimulatorActions.FORWARD.value])
            else:
                return action_list_to_one_hot(best_turn)
    ...
    def _est_max_grad_dir(self, goal_pos: np.array) -> np.array:
        ...
        if self.mode == "spath":
            points = self._sim.straight_spath_points(self._sim.get_agent_state().position, goal_pos)
            ...
            for _ in range(0, 360, self._sim.config.TURN_ANGLE):
                ...
                # If the best delta is within (1 - cos(TURN_ANGLE))% of the best delta (the step size),
                # then we almost certianly have
            ...
            self._sim.set_agent_state(current_pos, current_rotation, reset_sensors=False)
    ...
    def mode(self, new_mode):
        ...
        if new_mode == "spath":
            assert getattr(self._sim, "straight_spath_points", None) is not None
        self._mode = new_mode
