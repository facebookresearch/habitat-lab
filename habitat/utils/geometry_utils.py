...
def angle_between_quaternions(q1: qt.quaternion, q2: qt.quaternion) -> float:
    r"""Returns the angle (in radians) between two quaternions. This angle will
    always be positive.
    """
    q1_inv = np.conjugate(q1)
    dq = qt.as_float_array(q1_inv * q2)
...
def quaternion_from_coeff(coeffs: np.ndarray) -> qt.quaternion:
...
def agent_state_target2ref(
...
) -> Tuple[qt.quaternion, np.ndarray]:
    ...
    if not isinstance(ref_rotation, qt.quaternion):
        ref_rotation = quaternion_from_coeff(ref_rotation)
    ref_rotation = ref_rotation.normalized()

    if not isinstance(target_rotation, qt.quaternion):
