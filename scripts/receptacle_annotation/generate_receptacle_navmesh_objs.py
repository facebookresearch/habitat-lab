...
def save_navmesh_data(sim:habitat_sim.Simulator, output_dir:str) -> None:
    ...
    if sim.pathfinder.is_loaded:
        for island in range(sim.pathfinder.num_islands):
            ...
            export_navmesh_data_to_obj(filename=os.path.join(output_dir, f"{island}.obj"), vertex_data=vert_data, index_data=index_data)
    else:
        print("Cannot save navmesh data, no pathfinder loaded")

def export_navmesh_data_to_obj(filename:str, vertex_data:List[Any], index_data:List[int]) -> None:
    """
    ...
    with open(filename, "w") as f:
        ...
        for ix in range(int(len(index_data)/3)):
            ...
            file_data += "f " + str(index_data[ix*3]+1)+" "+str(index_data[ix*3+1]+1)+" "+ str(index_data[ix*3+2]+1)  + "\n"
        f.write(file_data)
...
def iteratively_export_all_scenes_navmesh(config_with_mm, recompute_navmesh=False):
    ...
    print("STAGES")
    stage_handles = config_with_mm.metadata_mediator.stage_template_manager.get_templates_by_handle_substring()
    ...
    for stage_handle in stage_handles:
        ...
        try:
            with habitat_sim.Simulator(config_with_mm) as sim:
                ...
                stage_output_dir = os.path.join(sim_settings["output_dir"], stage_filename.split(".")[0]+"/")
                os.makedirs(stage_output_dir, exist_ok=True)
                
                #export the render asset path for later use in Blender
                stage_template = sim.metadata_mediator.stage_template_manager.get_template_by_handle(stage_handle)
                render_asset_path = os.path.abspath(stage_template.render_asset_handle)
                render_asset_record_filepath = os.path.join(stage_output_dir, "render_asset_path.txt")
                ...
                if recompute_navmesh or not sim.pathfinder.is_loaded():
                    navmesh_filename = stage_filename.split(".")[0]+".navmesh"
                    sim.recompute_navmesh(sim.pathfinder, sim_settings["navmesh_settings"])
                    if os.path.exists(stage_directory):
                        sim.pathfinder.save_nav_mesh(stage_output_dir+navmesh_filename)
                    else:
                        failure_log.append((stage_handle, f"No target directory for navmesh: {stage_directory}"))
    ...
    print(f"Tried {len(stage_handles)-1} stages.") #manually decrement the "NONE" scene
    print("-------------------------------")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", dest="dataset", type=str, help="The SceneDataset config file.", required=True)
    parser.add_argument("--output-dir", dest="output_dir", type=str, default="navmeshes/", help="The output directory for the navmesh .OBJ files. Sub-directories will be created for each stage/scene.")
    parser.add_argument("--navmesh-settings", dest="navmesh_settings", type=str, default="", help="Optionally provide a path to a navmesh settings JSON file to use instead of the default settings.")
    ...
    if args.navmesh_settings != "":
        assert os.path.exists(args.navmesh_settings), f"Provided NavmeshSettings config file  '{args.navmesh_settings}' not found, aborting."
        assert args.navmesh_settings.endswith(".json"), "args.navmesh_settings must be a NavmeshSettings JSON file."
        sim_settings["navmesh_settings"].read_from_json(args.navmesh_settings)

    iteratively_export_all_scenes_navmesh(make_cfg_mm(sim_settings), recompute_navmesh=True)