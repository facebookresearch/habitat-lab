...
def get_mesh_receptacle_objects():
    ...
    """
    mesh_receptacles = [x for x in bpy.data.objects.values() if mesh_receptacle_id_string in x.name]
...
def load_island_mesh(datapath):
    ...
    if os.path.isfile(datapath):
        if datapath.endswith(".obj"):
            ...
            return #TODO: maybe support this again?
            bpy.ops.import_mesh.ply(filepath=datapath)
        else:
            print(f"Cannot process receptacles from this format '{datapath.split('.')[-1]}'. Use .ply or .obj")
            return
...
def load_render_asset_from_cache(render_asset_cache_path):
    assert os.path.isfile(render_asset_cache_path), f"'{render_asset_cache_path}' does not exist."
    assert render_asset_cache_path.endswith(".txt"), "must be a txt file containing only the render asset path."
    with open(render_asset_cache_path, "r") as f:
        ...
        if render_asset_path.endswith(".glb"):
            ...
            assert False, f"Import of filetype '{render_asset_path}' not supported currently, aborthing scene load."
            ...
            if mat.node_tree is not None:
                for node in mat.node_tree.nodes:
                    ...
                    if node.bl_label == "Principled BSDF":
                        #print(f" {dir(node)}")
                        node.inputs["Base Color"].default_value = (random.random(),random.random(),random.random(),1)

def get_receptacle_metadata(object_name, output_directory, mesh_relative_path=""):
    """
    Generate a JSON metadata dict for the provided receptacle object.
    """
    assert mesh_receptacle_id_string in object_name, f"Are you sure '{object_name}' is a mesh receptacle?"
    ...
    receptacle_info["position"] = [0,0,0]
    receptacle_info["rotation"] = [1,0,0,0]
    receptacle_info["scale"] = [1,1,1]
    receptacle_info["up"] = [0, 1, 0]

    # write the ply files and record the relative filepath
    
    bpy.ops.object.select_all(action='DESELECT')
    obj.select_set(True) 
    receptacle_info["mesh_filepath"] = mesh_relative_path + object_name + ".ply"
    bpy.ops.export_mesh.ply(
        filepath=os.path.join(output_directory, receptacle_info["mesh_filepath"]),
        use_selection = True,
        use_ascii = True,
...
def write_receptacle_metadata(output_filename, mesh_relative_path=""):
    ...
    for rec_obj in mesh_receptacles:
        user_defined[rec_obj.name] = get_receptacle_metadata(rec_obj.name, output_directory, mesh_relative_path)
...
path_to_receptacle_navmesh_assets = "/home/alexclegg/Documents/dev/habitat-lab/navmeshes/"
...
if mode == "read":
    ...
    print(stage_paths)
    assert len(stage_paths) > stage_index, f"Index {stage_index} out of range. {len(stage_paths)} available."
    ...
    if reload_scene:
        load_render_asset_from_cache(os.path.join(stage_paths[stage_index], "render_asset_path.txt"))
elif mode == "write":
    #write the results
    write_receptacle_metadata(output_filename = os.path.join(output_dir, "receptacle_metadata.json"), mesh_relative_path=mesh_relative_path)