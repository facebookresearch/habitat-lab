...
class HabDemoRunner:
    ...
    def step_env(self, action):
        start = time.time()
        obs = self.envs.step(action[0])  # type:ignore
    ...
    def do_time_steps(self):
        final_vid = []
        profile_sums = defaultdict(lambda: 0)  # type:ignore

        for step_idx in range(self.args.n_steps):
            actions = self.get_actions(step_idx)  # type:ignore
