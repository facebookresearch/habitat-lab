...
@hydra.main(
    version_base=None,
    config_path="../../examples/hitl/pick_throw_vr/config",
    config_name="pick_throw_vr",
)
def test_hitl_pickthrowvr(config):
    with pytest.raises(Exception) as e:
        config.habitat_hitl.experimental.headless.exit_after = 1
        hitl_main(
            config,
            lambda app_service: AppStateMinimal(app_service),
            test_flag=True,
        )
    assert "Pick Throw VR app failed" in str(e.value)


if __name__ == "__main__":
    register_hydra_plugins()
    test_hitl_pickthrowvr()
